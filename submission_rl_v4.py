"""Orbit Wars RL v4 - same architecture as v3, but trained on the
parity-correct orbit_wars env (planet rotation + swept-pair collision)
with lead-target features.

What changed vs v3 template:
  - PLANET_FEAT_DIM 12 -> 19: + (is_orbiting, orbit_phase, orbit_radius,
    x_at_t+15, y_at_t+15, x_at_t+30, y_at_t+30)
  - GLOBAL_FEAT_DIM 10 -> 11: + (angular_velocity / OMEGA_MAX)
  - encode_obs now reads obs['angular_velocity'] and derives orbit fields
    (is_orbiting, phase, radius, lead-target predictions) from each
    planet's (x, y) live position.

Lead-target features (v4.2) tell the policy where each orbiting planet
WILL BE at t+15 / t+30 steps. Without these the small transformer can't
