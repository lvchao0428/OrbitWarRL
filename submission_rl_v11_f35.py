"""Orbit Wars RL v11 f35 — f33 arch + 5 src-quality / v20-target-score feats.

Single delta vs f33: PLANET_FEAT_DIM 28 -> 33. Five new per-planet features
inject v20 heuristics so the src head can tell a stockpile from an empty shell
(DAY8 §11/§12 root cause: 55% of launches came from <=3-ship planets):

  [28] garrison_rank    -- my garrison / my_max_garrison (src "firepower hub")
  [29] safe_surplus_norm -- (my_garr - foe_soft) / my_total_garrison  (v20 surplus)
  [30] is_strong_source -- 1 if mine and garrison > my_avg_garrison
  [31] prod_per_need    -- prod^2 / need / 8         (v20 neutral_mfg)
  [32] v20_target_score -- (prod*20 + weak*30)*dist_decay - need*0.5  (v20 target_score)

Pair features + hard masks + PPO recipe IDENTICAL to f33 (5+4 pair, bin5 masks).

Must match ``orbit_wars_rl/features/encode.py`` + ``features/pair.py`` (f35):
  - PLANET_FEAT_DIM = 33  (f29 28 dims + 5 f35 dims)
  - FLEET_FEAT_DIM  = 10
  - GLOBAL_FEAT_DIM = 17
...
def encode_obs(obs: Dict[str, Any], player: int, step: int, episode_steps: int) -> dict:
    ...
    # [30] is_strong_source
    is_strong_source = (is_mine & (my_ships_f > my_avg_garrison)).astype(np.float32)
    ...
    eta_proxy = min_dist_to_mine / 2.0
    distance_decay = 1.0 / (1.0 + eta_proxy * 0.10)
    prod_value = prod_f * 20.0
    target_value = (prod_value + weak_target_score * 30.0) * distance_decay
    cost_term = need_est * 0.5
    v20_target_score = np.where(
        is_capturable_target > 0,
        np.clip((target_value - cost_term) / 100.0, -1.0, 1.0),
        0.0,
    )
...
def _dst_flip_block_mask(
    ...
    rem_src = float(max(int(remaining[src_idx]), 1))
    ships_at_bin5 = float(np.floor(rem_src * 0.7))
    garr_dst = planet_ships.astype(np.float32)
