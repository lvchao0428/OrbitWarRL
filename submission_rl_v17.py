"""Orbit Wars RL v17 — hist=50 + safe_emit + ETA-lead dst + pure terminal reward (Frog Parade style).

Deltas vs v13c:
  - GLOBAL_FEAT_DIM 24 -> 27  (+match_score_me, +match_score_opp, +match_progress)
  - Multi-match dims are always 0 at inference (Kaggle = single episode).

Feature dims:
  - PLANET_FEAT_DIM = 41
  - FLEET_FEAT_DIM  = 10
  - GLOBAL_FEAT_DIM = 427  (v13c 24 + 3 multi-match context)
  - MAX_FLEETS_PER_TURN = 8
  - DST_PAIR_DIM = 5
  - EMIT_PAIR_DIM = 6  (f35 4 + feasible_target_count_norm + surplus_ratio)
...
def agent(obs, config=None):
    ...
    global _STEP_COUNTER, _HOME_IDX, _GLOBAL_HIST
    try:
