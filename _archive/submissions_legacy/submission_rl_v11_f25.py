"""Orbit Wars RL v11 f25 — v11_k8 base + pct-inductive-bias features (Day7).

Must match ``orbit_wars_rl/features/encode.py`` (f25):
  - PLANET_FEAT_DIM 22 -> 28: + flip_cost, friendly_surplus, capturable_bin3/5,
    needed_pct_norm, weak_target_score
  - FLEET_FEAT_DIM 8 -> 10: + target_dist_norm, target_garrison_norm
  - GLOBAL_FEAT_DIM 14 -> 17: + max_garr_norm, n_weak_targets_norm,
    ships_to_capture_all_weak_norm
  - MAX_FLEETS_PER_TURN = 8

Use this template when exporting v11_f25 checkpoints (planet=28, global=17).
