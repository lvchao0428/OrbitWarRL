"""Orbit Wars RL v3 - multi-fleet autoregressive policy, numpy-only inference.

Architecture (mirrors orbit_wars_rl.net.model.ActorCritic with
max_fleets_per_turn>1):
  encoder: per-entity projection + type embed + 2 pre-norm self-attn blocks
  src_head: dense(1) over planet embeddings; masked to owned planets with
            remaining ships > 0 (running reserved_ships)
  dst_head: cross-attention conditioned on chosen src; masked only to padding
            (owned planets allowed as dst = reinforcement; same-owner arrivals
            stack ships per env dynamics)
  pct_head: MLP(src, dst, global) -> 4-bin softmax (25/50/75/100% of available)
  emit_head: MLP(global, planet_pool, step_one_hot) -> 2-bin (stop|continue)
  value_head: MLP(global, planet_pool, fleet_pool) -> scalar (not used at inference)

Each turn we autoregressively unroll up to K=8 launches: emit/stop -> src ->
dst -> pct, with reserved_ships tracking to prevent over-spending a single
source planet across multiple launches in the same turn.

The weights are stored at the bottom of this file as a single base64-compressed
``.npz`` payload (~600KB encoded). They are unpacked once on module load;
``agent()`` runs the K-step autoregressive numpy forward per turn (a handful
of ms on Kaggle's CPU box for 40 planets / 128 fleets).
"""
