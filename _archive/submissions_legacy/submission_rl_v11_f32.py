"""Orbit Wars RL v11 f32 — speed-aware pair features + relaxed masks (Day7 Phase C).

Must match ``orbit_wars_rl/features/encode.py`` + ``features/pair.py`` (f32):
  - PLANET_FEAT_DIM = 28  (same as f25-f31)
  - FLEET_FEAT_DIM  = 10
  - GLOBAL_FEAT_DIM = 17
  - MAX_FLEETS_PER_TURN = 8
  - DST_PAIR_DIM = 7  (+ speed_norm, +eta_norm; dst_fc1 input 2*d+8)
  - EMIT_PAIR_DIM = 5  (+feasible_avg_speed_norm)

Hard masks (f32, relaxed from f31 bin5 to bin3):
  EmitHead  -- ``EMIT_HARD_STOP=1``: worth_it=0 (bin3) -> force stop (step>0)
  DstHead   -- ``F32_HARD_MASKS=1``: block dst where floor(rem*0.4) <= garr[dst]

``F29_SIGNALS = 1`` + ``F32_HARD_MASKS = 1`` for export/replay routing.
...
EMIT_PAIR_DIM = 5
EMIT_HARD_STOP = 1
FLIP_BIN_FRAC = 0.40  # f32: bin3 threshold (relaxed from f31's 0.70)
MAX_SPEED = 6.0
...
def _dst_pair_features(planet_x, planet_y, planet_ships, planet_mask,
    ...
    # f32: speed-aware features.
    ships_at_flip = np.float32(np.floor(rem_src * FLIP_BIN_FRAC))
    spd_norm = float(np.clip(
        (1.0 + (MAX_SPEED - 1.0) * (np.log(max(ships_at_flip, 1.0)) / LOG_1000) ** 1.5) / MAX_SPEED,
        0.0, 1.0,
    ))
    spd = spd_norm * MAX_SPEED
    eta_raw = dist / max(spd * BOARD, 1.0)
    eta_norm = np.clip(eta_raw, 0.0, 1.0).astype(np.float32)
    speed_norm_arr = np.where(self_pair, np.float32(0.0), np.float32(spd_norm))
    eta_norm = np.where(self_pair, np.float32(0.0), eta_norm)

    pair_feats = np.stack(
        [dist_norm, sun_risk, ships_needed_norm, pair_flip_bin5, pair_margin_norm,
         speed_norm_arr, eta_norm], axis=-1
    ).astype(np.float32)
...
def _dst_flip_block_mask(
    ...
    rem_src = float(max(int(remaining[src_idx]), 1))
    ships_at_flip = float(np.floor(rem_src * FLIP_BIN_FRAC))
    garr_dst = planet_ships.astype(np.float32)
    flip_ok = ships_at_flip > garr_dst
    return (
...
def _emit_pair_globals(planet_x, planet_y, planet_ships, planet_mask, my_mask,
    ...
    ships_at_flip_src = np.floor(rem_my * np.float32(FLIP_BIN_FRAC))  # f32: bin3
    garr_dst = planet_ships.astype(np.float32)

    margin = ships_at_flip_src[:, None] - garr_dst[None, :]
    ...
    # f32: avg speed across feasible pairs.
    speed_per_src = np.array([
        float(np.clip(
            (1.0 + (MAX_SPEED - 1.0) * (np.log(max(float(s), 1.0)) / LOG_1000) ** 1.5) / MAX_SPEED,
            0.0, 1.0,
        )) for s in ships_at_flip_src
    ], dtype=np.float32)
    speed_2d = speed_per_src[:, None]
    speed_masked = np.where(feasible, speed_2d, np.float32(0.0))
    n_feasible = float(feasible.astype(np.float32).sum())
    speed_sum = float(speed_masked.sum())
    avg_speed = speed_sum / max(n_feasible, 1.0) if n_feasible > 0 else 0.0
    avg_speed_norm = float(min(max(avg_speed, 0.0), 1.0))
