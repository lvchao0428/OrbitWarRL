"""Orbit Wars RL v11 f33 — f31 arch + top-player PPO hparams, long train.

IDENTICAL architecture to f31 (5+4 pair, hard masks bin5). The only difference
vs f31 is the *training recipe* (single ent_coef=0.05, lr=3e-5, ppo_epochs=1,
clip_eps=0.20, value_coef=0.5, 2000 updates) -- inference code is unchanged, so
this template is a byte-for-byte copy of submission_rl_v11_f31.py except this
header. Used by quick_replay.sh to export f33 checkpoints.

Must match ``orbit_wars_rl/features/encode.py`` + ``features/pair.py`` (f29/f31):
  - PLANET_FEAT_DIM = 28  (same as f25-f29)
  - FLEET_FEAT_DIM  = 10
  - GLOBAL_FEAT_DIM = 17
