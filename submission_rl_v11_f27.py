"""Orbit Wars RL v11 f27 — f26 pair heads + min_bin_norm pct fix (Day7).

Must match ``orbit_wars_rl/features/encode.py`` + ``features/pair.py`` (f27):
  - PLANET_FEAT_DIM = 28  (same as f25/f26)
  - FLEET_FEAT_DIM  = 10  (same as f25/f26)
  - GLOBAL_FEAT_DIM = 17  (same as f25/f26)
...
Pair head augmentations (K-loop, per step):
  ...
  PctHead   += 2 scalars for chosen (src,dst) [min_bin_norm, pair_flip_bin5].
              f27 fix: ``min_bin_norm`` = smallest bin index that flips dst / 7.
              (f26 used (garr+1)/remaining which correlated with bin0 spam.)

``MIN_BIN_PCT_FEAT = 1`` distinguishes this template from f26 for export/replay routing.
...
def _pct_pair_features(garr_dst, remaining_src):
    ...
    pct_bins = np.array([0.10, 0.20, 0.30, 0.40, 0.55, 0.70, 0.85, 1.00], dtype=np.float32)
    ships_at_bins = np.floor(rem * pct_bins)
    flip_at_bin = ships_at_bins > float(garr_dst)
    if bool(flip_at_bin.any()):
        min_bin = int(np.argmax(flip_at_bin))
    else:
        min_bin = len(pct_bins) - 1
    min_bin_norm = min_bin / (len(pct_bins) - 1)
