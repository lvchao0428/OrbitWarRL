...
def _make_state(
    ...
    return EnvState(
        ...
        init_planet_owner=jnp.asarray(owner),
...
def test_single_move_roundtrip():
    ...
    # pct: 10/20 = 0.5; 16-bin table hits 0.50 exactly at idx 9
    assert t["pct"][0] == 9, f"expected pct_bin=9 (.50), got {t['pct'][0]}"
...
def test_two_moves_same_src_reserved():
    ...
    # first move: 30/100 = 0.30 -> 16-bin idx 5 -> decoded floor(100*.30)=30. reserved=30.
    # second move: 30/(100-30)=30/70 = 0.4286 -> closest bin .45 (idx 8).
    ...
    assert t["pct"][0] == 5, f"expected pct[0]=5 (.30), got {t['pct'][0]}"
    assert t["src"][1] == 0 and t["dst"][1] == 2
    assert t["pct"][1] == 8, f"expected pct[1]=8 (.45), got {t['pct'][1]}"
