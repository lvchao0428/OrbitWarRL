"""v27 integration smoke: frontier encode, shuffle reward env step, resume, export.

Run: ``python -m orbit_wars_rl.scripts.smoke_v27``
"""
...
def _check_encode() -> None:
    ...
    assert float(obs.planet_feats[0, 26]) >= 0.0
    print("[OK ] encode shapes + v27 dim26 frontier slot")
...
def _check_frontier_env_step() -> None:
    ...
    print(f"[OK ] v27 env step reward finite ({r_f:+.4f})")
...
def _check_resume(ckpt_path: Path) -> None:
    ...
    cfg = TrainConfig(
        ...
        eval_every=0,
        ckpt_dir=str(tempfile.mkdtemp(prefix="smoke_v27_")),
        ckpt_every=0,
        log_every=1,
        d_model=256,
        n_layers=4,
...
def _check_export(ckpt_path: Path, root: Path) -> None:
    ...
    if not template.is_file():
        ...
        return
    out = root / "tmp_smoke_v27_submission.py"
    cmd = [
        sys.executable,
        "-m",
...
def main() -> int:
    ...
    for name in (
        "ckpt_multi_action_v26_roi/ckpt_latest.pkl",
        "ckpt_multi_action_v26_roi/ckpt_009599.pkl",
        "ckpt_multi_action_v25_extend/ckpt_009599.pkl",
    ):
        ckpt = Path(os.environ.get("V27_RESUME_CKPT", name))
