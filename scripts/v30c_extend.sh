#!/usr/bin/env bash
# v30c: capture_ready features; from v30b u1599 (~40min for 800u)
set -euo pipefail
...
NUM_UPDATES="${1:-800}"
LOGDIR="logs/v30c_extend_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOGDIR"
...
echo "═══════════════════════════════════════════════════"
echo " v30c — capture_ready + emit_urgency from v30b u1599"
echo "═══════════════════════════════════════════════════"
echo " init     : $INIT"
...
"$PY" -m orbit_wars_rl.scripts.train \
  --config orbit_wars_rl/configs/multi_action_v30c_capture_ready.yaml \
  --log-dir "$LOGDIR" \
