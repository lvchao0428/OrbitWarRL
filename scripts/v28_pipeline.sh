#!/usr/bin/env bash
# v28 fast pipeline: BC 200g -> top10 buffer -> epw30 -> PPO ~3.5h -> post.
#
# Usage:
#   bash scripts/v28_pipeline.sh [--updates N] [--skip-post] [--from gateN]
set -euo pipefail
...
_gate0_smoke() {
  ...
  bash scripts/v28_smoke.sh 2>&1 | tee -a "$LOG_FILE"
...
_gate_bc_train() {
  ...
  echo "[pipeline] Gate2 BC train epw3.0 -> ckpt_bc_v28_epw30"
  mkdir -p ckpt_bc_v28_epw30
  "$RPY" -m orbit_wars_rl.bc.train_bc \
    --data data/bc_v20_self_200g.npz \
    --epochs 5 --batch-size 256 --lr 3e-4 \
    --emit-pos-weight 3.0 \
    --out ckpt_bc_v28_epw30/ckpt_final.pkl \
    2>&1 | tee -a "$LOG_FILE"
  test -f ckpt_bc_v28_epw30/ckpt_final.pkl || { _write_status gate2_bc_train 1 gate2_bc_train; exit 1; }
  echo "[pipeline] Gate2 quick_replay 5g sanity"
  NUM_GAMES=5 JAX_PLATFORMS=cpu CUDA_VISIBLE_DEVICES="" \
    bash scripts/quick_replay.sh ckpt_bc_v28_epw30/ckpt_final.pkl v28_bc_epw30_gate \
    2>&1 | tee -a "$LOG_FILE"
...
_gate_ppo_train() {
  ...
  echo "[pipeline] Gate3 v28 PPO train ($UPDATES updates)"
  bash scripts/v28_extend.sh "$UPDATES" 2>&1 | tee -a "$LOG_FILE"
...
_gate_post_train() {
  ...
  bash scripts/v28_post_train.sh 2>&1 | tee -a "$LOG_FILE"
  _write_status gate4_post_train 0
}

main() {
  exec > >(tee -a "$LOG_FILE") 2>&1
  echo "=== v28 pipeline start $(date -Iseconds) updates=$UPDATES skip_post=$SKIP_POST from=$FROM_GATE ==="
  ...
  echo "=== v28 pipeline DONE $(date -Iseconds) ==="
}
