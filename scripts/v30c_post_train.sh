#!/usr/bin/env bash
# v30c post-train: seed=0 replay
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
export PYTHON="${PYTHON:-python3}" JAX_PLATFORMS=cpu CUDA_VISIBLE_DEVICES="" ORBITWARS_SKIP_PARITY=1
CKPT_DIR="ckpt_multi_action_v30c_capture"
mkdir -p logs/replay_analyze logs/replay_html logs/v30c_h2h
U799=799
CKPT="$CKPT_DIR/ckpt_$(printf '%06d' "$U799").pkl"
[ -f "$CKPT" ] || CKPT="$CKPT_DIR/eval/ckpt_eval_u$(printf '%06d' "$U799").pkl"
[ -f "$CKPT" ] || CKPT="$(ls -t "$CKPT_DIR"/ckpt_*.pkl 2>/dev/null | head -1)"
if [ -n "${CKPT:-}" ] && [ -f "$CKPT" ]; then
  TAG="v30c_u$(basename "$CKPT" .pkl | sed 's/ckpt_//')"
