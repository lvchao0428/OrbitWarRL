#!/usr/bin/env bash
# v27 post-train: h2h20, win HTML, DAY22 progress stub.
set -euo pipefail
...
CKPT_DIR="ckpt_multi_action_v27_frontier"
mkdir -p logs/replay_analyze logs/replay_html logs/v27_h2h logs/v27_win_replays docs
...
updates = []
for p in glob.glob("ckpt_multi_action_v27_frontier/ckpt_*.pkl"):
    m = re.search(r"(\d{6})", Path(p).name)
    if m:
        updates.append(int(m.group(1)))
for p in glob.glob("ckpt_multi_action_v27_frontier/eval/ckpt_eval_u*.pkl"):
...
if [ ${#CKPTS[@]} -eq 0 ]; then
  CKPTS=(199 399 799 1199 3999 7999 11999)
fi
...
for U in "${CKPTS[@]}"; do
  ...
  if [ ! -f "$JSON" ]; then
    echo "[analyze] $TAG ..."
    bash scripts/quick_replay.sh "$CKPT" "$TAG" >/tmp/qr_v27_${U}.log 2>&1 || {
      echo "WARN quick_replay failed $TAG"; tail -5 /tmp/qr_v27_${U}.log
    }
  fi
done
...
H2H_LOG="logs/v27_h2h/h2h20_summary.txt"
: > "$H2H_LOG"
for U in "${CKPTS[@]: -3}"; do
  CKPT="$CKPT_DIR/ckpt_$(printf '%06d' "$U").pkl"
...
WINS_FILE="logs/v27_win_replays/win_seeds.txt"
: > "$WINS_FILE"
"$PYTHON" - <<'PY'
import json, glob
from pathlib import Path
...
wins = []
for p in sorted(glob.glob("logs/replay_analyze/v27_*_vs_v20.json")):
    d = json.loads(Path(p).read_text())
    tag = p.split("/")[-1].replace("_vs_v20.json", "")
    u = tag.replace("v27_u", "").replace("v27_h2h_u", "")
...
Path("logs/v27_win_replays/win_seeds.txt").write_text(
...
while read -r U SEED TAG; do
  ...
  CKPT="$CKPT_DIR/ckpt_$(printf '%06d' "$U").pkl"
  [ -f "$CKPT" ] || CKPT="$CKPT_DIR/eval/ckpt_eval_u$(printf '%06d' "$U").pkl"
  OUT="logs/replay_html/v27_u${U}_s${SEED}"
...
echo ""
echo "=== Phase 5: DAY22 progress doc ==="
DOC="docs/DAY22_PROGRESS.zh.md"
TRAIN_LOG="$(ls -t logs/v27_extend_*/train.log 2>/dev/null | head -1)"
{
  echo "# DAY22 进展 — v27 frontier"
  echo ""
  echo "生成时间: $(date -Iseconds)"
  echo ""
  echo "## 训练"
  echo "- log: \`${TRAIN_LOG:-logs/v27_extend_*/train.log}\`"
  echo "- ckpt_dir: \`ckpt_multi_action_v27_frontier/\`"
  echo ""
  ...
  echo "| friendly_shuffle | replay 目测 ↓ |"
  echo "| vs-random | ≥ 95% |"
} > "$DOC"
echo "wrote $DOC"
echo "=== v27 post-train done ==="
