#!/usr/bin/env bash
# v27 fast pipeline: BC 200g -> top10 buffer -> epw30 -> PPO ~2.5h -> post.
#
# Usage:
#   bash scripts/v27_pipeline.sh [--updates N] [--skip-post] [--from gateN]
set -euo pipefail
...
_gate0_smoke() {
  ...
  bash scripts/v27_smoke.sh 2>&1 | tee -a "$LOG_FILE"
  _write_status gate0_smoke 0
}

_gate_bc_collect() {
  _write_status gate1_bc_collect
  echo "[pipeline] Gate1b top10 winner states (background, CPU)"
  bash scripts/bc_collect_v27_top10.sh 2>&1 | tee -a "$LOG_FILE" &
  TOP10_PID=$!
  echo "[pipeline] Gate1 BC collect 4x50 -> bc_v20_self_200g.npz"
  bash scripts/bc_collect_parallel.sh 4 50 data/bc_v20_self_200g.npz 2>&1 | tee -a "$LOG_FILE"
  wait "$TOP10_PID" 2>/dev/null || echo "[pipeline] WARN top10 collect failed — PPO runs without buffer"
  "$RPY" - <<'PY' || { _write_status gate1_bc_collect 1 gate1_bc_collect; exit 1; }
import numpy as np
d = np.load("data/bc_v20_self_200g.npz")
n = d["src"].shape[0]
...
assert n >= 50_000, f"too few samples: {n}"
assert pf[-1] == 63, pf
assert gf[-1] == 427, gf
assert 45 <= hold <= 75, hold
assert 12 <= e2p <= 35, e2p
...
_gate_bc_train() {
  ...
  echo "[pipeline] Gate2 BC train epw3.0 -> ckpt_bc_v27_epw30"
  mkdir -p ckpt_bc_v27_epw30
  "$RPY" -m orbit_wars_rl.bc.train_bc \
    --data data/bc_v20_self_200g.npz \
    --epochs 5 --batch-size 256 --lr 3e-4 \
    --emit-pos-weight 3.0 \
    --out ckpt_bc_v27_epw30/ckpt_final.pkl \
    2>&1 | tee -a "$LOG_FILE"
  test -f ckpt_bc_v27_epw30/ckpt_final.pkl || { _write_status gate2_bc_train 1 gate2_bc_train; exit 1; }
  echo "[pipeline] Gate2 quick_replay 5g sanity"
  NUM_GAMES=5 JAX_PLATFORMS=cpu CUDA_VISIBLE_DEVICES="" \
    bash scripts/quick_replay.sh ckpt_bc_v27_epw30/ckpt_final.pkl v27_bc_epw30_gate \
    2>&1 | tee -a "$LOG_FILE"
...
_gate_ppo_train() {
  ...
  echo "[pipeline] Gate3 v27 PPO train ($UPDATES updates)"
  bash scripts/v27_extend.sh "$UPDATES" 2>&1 | tee -a "$LOG_FILE"
  TRAIN_LOG="$(ls -t logs/v27_extend_*/train.log 2>/dev/null | head -1)"
...
_gate_post_train() {
  ...
  bash scripts/v27_post_train.sh 2>&1 | tee -a "$LOG_FILE"
  _write_status gate4_post_train 0
}

main() {
  exec > >(tee -a "$LOG_FILE") 2>&1
  echo "=== v27 pipeline start $(date -Iseconds) updates=$UPDATES skip_post=$SKIP_POST from=$FROM_GATE ==="
  ...
  echo "=== v27 pipeline DONE $(date -Iseconds) ==="
}
