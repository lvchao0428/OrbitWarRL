#!/usr/bin/env bash
# v14e Phase C long-train (10k updates) — run after B confirm passes
set -euo pipefail
...
READY_JSON="logs/v14e_c_longtrain.ready.json"
if [ ! -f "$READY_JSON" ]; then
  echo "[v14e_c_long] ERROR: $READY_JSON not found — Phase B hasn't passed yet."
  exit 1
fi

RESUME_CKPT=$(python3 -c "import json; print(json.load(open('$READY_JSON'))['resume_ckpt'])")
if [ -z "$RESUME_CKPT" ] || [ "$RESUME_CKPT" = "null" ]; then
  echo "[v14e_c_long] ERROR: no resume_ckpt in $READY_JSON"
  exit 1
fi

echo "[v14e_c_long] resume from $RESUME_CKPT"

export ORBITWARS_SHAPING_SCALE=0.0
export ORBITWARS_SHAPING_HOLD_BONUS=0.0
export ORBITWARS_SHAPING_ANTI_HOARD=0.015
...
mkdir -p ckpt_multi_action_v14e_c_long logs

nohup "$PY" -m orbit_wars_rl.scripts.train \
  --config orbit_wars_rl/configs/multi_action_v14e_phase_c_long.yaml \
  --log-dir logs/v14e_c_long \
  --num-updates 10000 \
  --resume-from "$RESUME_CKPT" \
  >> logs/v14e_phase_c_long.log 2>&1 &

echo "pid=$! log=logs/v14e_phase_c_long.log"
