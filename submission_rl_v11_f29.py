"""Orbit Wars RL v11 f29 — full signal stack (Day7).

Must match ``orbit_wars_rl/features/encode.py`` + ``features/pair.py`` (f29):
  - PLANET_FEAT_DIM = 28  (same as f25-f28)
  - FLEET_FEAT_DIM  = 10
  - GLOBAL_FEAT_DIM = 17
...
Pair head augmentations (K-loop, per step):
  DstHead   += 5 scalars per candidate dst (+ pair_margin_norm)
  EmitHead  += emit_worth_it (replaces n_feasible_pairs), best_margin, home/total ratio
  PctHead   += min_bin hard mask (bins below min_flip_bin -> -inf)

``F29_SIGNALS = 1`` distinguishes this template from f27 for export/replay routing.
``EMIT_HARD_STOP = 0`` here; f29b template sets ``EMIT_HARD_STOP = 1``.
